function [corrIMean,corrNIMean,corrIPeak,corrNIPeak] = corrAtCuesNoShuffleAllIdxFovGrid(useIdx,useFolders,allDistancesIGroup,allDistancesNIGroup)

%similar to corrAtCuesNoShuffleAllIdxFov but for grid cells: so does not
%use cue cell threshold to load specific cue cell data (identified based on
%the percentile of cue cell thresholds). instead, it loads cueCells.mat

%the difference between this one and corrAtCuesNoShuffle is that
...
for n=1:length(useFolders)
    ...
    [~,useCellIdx,~]=intersect(data.useIdx,cells);
