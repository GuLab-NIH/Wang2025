function [corrIMean,corrNIMean,corrIPeak,corrNIPeak] = corrAtCuesNoShuffleAllIdxFov_inputBinAwayBin(useIdx,useFolders,allDistancesIGroupWithCueBin,allDistancesNIGroupWithCueBin,expandBin,awaybin)
%similar to corrAtCuesNoShuffleAllIdxFov_inputBin but included awayBin so
%that the bins can be certain bins away from cue center

%the difference between this one and corrAtCuesNoShuffle_inputBin is that
%it taks all shuffle Idx of all fovs, but the above one only takes one
%shuffle in one FOV
%useIdx is a cell structure, each cell is one fov. within each cell, there
%is a matrix: number of rows are the NCell in each shuffle, and number of
%columns are the number of shuffles

%very simliar to corrAtCuesNoShuffle but input bin: for grid cells, which
%do not have peak bins
...
for n=1:length(useFolders)
    ...
    for nShuffles=1:size(useIdx{n},2);
        cells=useIdx{n}(:,nShuffles);
...
for m=1:length(allDistancesNIGroupWithCueBin);%every distance 
        ...
        % corrNIPeak{m}={};
            ...
            % corrNIMean{m}{i}=[];
            % corrNIPeak{m}{i}=[];
            % cue1=allDistancesNIGroupWithCueBin{m}(i,1);
            % cue2=allDistancesNIGroupWithCueBin{m}(i,2);
