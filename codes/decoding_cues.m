function [decd] = decoding_cues(dfof,abf,trackStart,trackEnd,cue1Bin,cue2Bin)

%this code is modified from decoding.m but this is not to decode the whole
%track, but between cues.
...
%     return
    ...
    dfofMTemp{n}(r,binNumber)=nanmean(FcuseThisRun(nThisRun==binNumber&fastEnoughThisRun));
    end
...
cue1Temp=avgTemp(:,cue1Bin);
cue2Temp=avgTemp(:,cue2Bin);
cue12Temp=[cue1Temp cue2Temp];%the first 4 columns are cue1, the second 4 are cue 2
%remaining even runs
...
nThisRun=nbin(i);
cue1=[];
for ii=1:length(cue1Bin);
    thisi=find(nThisRun==cue1Bin(ii));
    cue1=[cue1;thisi];
end
cue1i=i(cue1);

cue2=[];
for ii=1:length(cue2Bin);
    thisi=find(nThisRun==cue2Bin(ii));
    cue2=[cue2;thisi];
end
cue2i=i(cue2);
testUseIndicesCue1=[testUseIndicesCue1 cue1i];
testUseIndicesCue2=[testUseIndicesCue2 cue2i];
end
...
dfofTestCue1=dfof(testUseIndicesCue1,:)';%row is cell, column is time
dfofTestCue2=dfof(testUseIndicesCue2,:)';%row is cell, column is time

testYBinsCue1=nbin(testUseIndicesCue1);
testYBinsCue2=nbin(testUseIndicesCue2);
...
%%%%%%%%%%%%%%%%%%%%%%%%% DECODE
decodeYBinsCue1=[];%decoded bins
decodeYBinsCue2=[];

decodeCuesCue1=[];%decode cues; severa bins belong to the same cue
decodeCuesCue2=[];

% allBins=[1:1:trackLength/binWidth];
tempBins=[cue1Bin cue2Bin];%real bins of the temp data
tempCues=[ones(1,length(cue1Bin)) 2*ones(1,length(cue2Bin))];%indices of which bin belongs to which cue
for n=1:size(dfofTestCue1,2);
    disp(n)
    c=[];
    test=dfofTestCue1(:,n);
    for m=1:size(cue12Temp,2);
        c(m)=corr(test,cue12Temp(:,m));
    end

    [~,idecode]=max(c,[],'omitnan');
    decodeYBinsCue1(n,1)=tempBins(idecode);%decoded bins: which bin
    decodeCuesCue1(n,1)=tempCues(idecode); %decoded cues; which cue: the first half numbers of bins belong to cue1, the second half numbers belong to cue 2
end

for n=1:size(dfofTestCue2,2);
    disp(n)
    c=[];
    test=dfofTestCue2(:,n);
    for m=1:size(cue12Temp,2);
    ...
    [~,idecode]=max(c,[],'omitnan');
    decodeYBinsCue2(n,1)=tempBins(idecode);
    decodeCuesCue2(n,1)=tempCues(idecode); 
end
...
decd=[];
decd.dfofTestCue1=dfofTestCue1;
decd.dfofTestCue2=dfofTestCue2;
decd.cue1Bin=cue1Bin;
decd.cue2Bin=cue2Bin;
decd.cue1Temp=cue1Temp;
decd.cue2Temp=cue2Temp;
decd.testYBinsCue1=testYBinsCue1;%initial bins of the test dfof
decd.testYBinsCue2=testYBinsCue2;

decd.decodeYBinsCue1=decodeYBinsCue1; %decoded bins
decd.decodeYBinsCue2=decodeYBinsCue2;

decd.decodeYBinsCue1=decodeCuesCue1; %decoded cues (several bins can belong to the same cue
decd.decodeYBinsCue2=decodeCuesCue2;

decd.perCorrectCue1=length(find(decodeCuesCue1==1))/length(decodeCuesCue1);
decd.perCorrectCue2=length(find(decodeCuesCue2==2))/length(decodeCuesCue2);
decd.perCorrectAll=(length(find(decodeCuesCue1==1))+length(find(decodeCuesCue2==2)))/(length(decodeCuesCue1)+length(decodeCuesCue2));
% save('decd.mat','decd')
end
